...
/**
 * Get emails of all VYA Insiders: paid subscribers + referral insiders (2+ friends invited).
 */
export async function getInsiderUserEmails(): Promise<string[]> {
 await initMembershipColumns();
 const sql = neon(getDatabaseUrl());
 const rows = await sql`
 SELECT LOWER(email) AS email FROM users WHERE is_member = TRUE AND email IS NOT NULL
 UNION
 SELECT LOWER(email) AS email FROM pilot_access WHERE is_insider = TRUE AND email IS NOT NULL
 `;
