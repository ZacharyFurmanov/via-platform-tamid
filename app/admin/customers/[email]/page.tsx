                    ...
                    <div key={i} style={{ background: "#fff", border: `1px solid ${BORDER}`, borderRadius: 8, overflow: "hidden" }}>
                      ...
                      {expandedSessions.has(i) && (
                        <div style={{ borderTop: `1px solid ${BORDER}`, padding: "0 16px 12px" }}>
                          {sess.events.map((e, j) => (
                            <div key={j} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "8px 0", borderBottom: j < sess.events.length - 1 ? `1px solid #f4f4f5` : "none" }}>
                              <div style={{ display: "flex", gap: 8, alignItems: "flex-start" }}>
                                <span style={{ fontSize: 12, marginTop: 1, flexShrink: 0 }}>
                                  {e.type === "click" ? "↗" : e.type === "view" ? "👁" : e.type === "favorite" ? "♥" : "🛍"}
                                </span>
                                <div>
                                  <p style={{ fontSize: 13, color: DARK, margin: 0 }}>{e.label}</p>
                                  {e.store && (
